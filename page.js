              ...
              <div key={idx} style={{...card}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
                  <div>
                    ...
                    <input type="number" min="1" max="30" value={city.days} onChange={e=>updateCity(idx,'days',e.target.value)}
                ...
                <div style={{marginBottom:10}}>
                  <div style={{fontSize:11,color:'#aaa',marginBottom:6}}>🏨 숙소 (체크인 기준)</div>
                  {city.hotels.map((hotel,hIdx)=>(
                    <div key={hIdx} style={{display:'flex',gap:6,marginBottom:6,alignItems:'center'}}>
                      ...
                      <input type="text" placeholder="숙소명 또는 지역 (예: 술탄아흐메트)"
                  ...
                  <button onClick={()=>addHotel(idx)} style={{fontSize:12,color:'#888',background:'none',border:'1px dashed #ddd',borderRadius:8,padding:'6px 12px',cursor:'pointer',marginTop:2}}>+ 숙소 추가 (체크인 날짜 변경)</button>
                </div>

                <div>
                  <div style={{fontSize:11,color:'#aaa',marginBottom:6}}>⏰ 일정 시작</div>
          ...
          <>
            <div style={{display:'flex',borderBottom:'1px solid #f0f0f0',overflowX:'auto'}}>
              {[['plan','📅 일정'],['map','🗺️ 지도'],['food','🍽️ 식당'],['book','🎟️ 예약'],['tips','💡 TIP']].map(([id,label])=>(
                <button key={id} onClick={()=>setActiveTab(id)} style={{flex:1,padding:'12px 4px',fontSize:11,color:activeTab===id?'#111':'#aaa',border:'none',background:'none',cursor:'pointer',borderBottom:activeTab===id?'2px solid #111':'2px solid transparent',fontWeight:activeTab===id?600:400,whiteSpace:'nowrap',minWidth:60}}>
                  {label}
                </button>
              ))}
            </div>
                  ...
                  <div style={card}>
                    ...
                    <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:10}}>
                      {[['VISA',c.payment?.visa],['Master',c.payment?.master],['Amex',c.payment?.amex]].map(([name,ok])=>(
                        <span key={name} style={{fontSize:12,padding:'4px 10px',borderRadius:20,background:ok?'#E8F5E9':'#FFEBEE',color:ok?'#2E7D32':'#C62828',fontWeight:500}}>
                          {ok?'✓':'✗'} {name}
                        </span>
                      ))}
